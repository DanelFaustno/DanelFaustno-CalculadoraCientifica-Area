...
namespace CalculadoraCientifica
{
    public partial class Form1 : Form
    {
        ...
        // Método para calcular el área según la figura seleccionada
        // Evento Click para calcular el área
        // Evento Click para calcular el área
        private void CalculateButton_Click(object sender, EventArgs e)
        {
            double area = 0;

            if (checkBox1.Checked) // Círculo
            {
                if (ValidarEntrada(inputTextBox1.Text))
                {
                    double radio = double.Parse(inputTextBox1.Text);
                    if (radio > 0)
                        area = Math.PI * Math.Pow(radio, 2);
                    else
                        MostrarError("El radio debe ser mayor a 0.");
                }
            }
            else if (checkBox2.Checked) // Cuadrado
            {
                if (ValidarEntrada(inputTextBox1.Text))
                {
                    double lado = double.Parse(inputTextBox1.Text);
                    if (lado > 0)
                        area = lado * lado;
                    else
                        MostrarError("El lado debe ser mayor a 0.");
                }
            }
            else if (checkBox3.Checked) // Rectángulo
            {
                if (ValidarEntrada(inputTextBox1.Text) && ValidarEntrada(inputTextBox2.Text))
                {
                    double ancho = double.Parse(inputTextBox1.Text);
                    double alto = double.Parse(inputTextBox2.Text);
                    if (ancho > 0 && alto > 0)
                        area = ancho * alto;
                    else
                        MostrarError("El ancho y el alto deben ser mayores a 0.");
                }
            }
            else if (checkBox9.Checked) // Triángulo
            {
                if (ValidarEntrada(inputTextBox1.Text) && ValidarEntrada(inputTextBox2.Text))
                {
                    double baseTri = double.Parse(inputTextBox1.Text);
                    double altura = double.Parse(inputTextBox2.Text);
                    if (baseTri > 0 && altura > 0)
                        area = (baseTri * altura) / 2;
                    else
                        MostrarError("La base y la altura deben ser mayores a 0.");
                }
            }
            else if (checkBox8.Checked) // Trapecio
            {
                if (ValidarEntrada(inputTextBox1.Text) && ValidarEntrada(inputTextBox2.Text) && ValidarEntrada(inputTextBox3.Text))
                {
                    double baseMayor = double.Parse(inputTextBox1.Text);
                    double baseMenor = double.Parse(inputTextBox2.Text);
                    double altura = double.Parse(inputTextBox3.Text);
                    if (baseMayor > 0 && baseMenor > 0 && altura > 0)
                        area = ((baseMayor + baseMenor) * altura) / 2;
                    else
                        MostrarError("Las bases y la altura deben ser mayores a 0.");
                }
            }
            else if (checkBox7.Checked) // Pentágono
            {
                if (ValidarEntrada(inputTextBox1.Text) && ValidarEntrada(inputTextBox2.Text))
                {
                    double lado = double.Parse(inputTextBox1.Text);
                    double apotema = double.Parse(inputTextBox2.Text);
                    if (lado > 0 && apotema > 0)
                        area = (5 * lado * apotema) / 2;
                    else
                        MostrarError("El lado y el apotema deben ser mayores a 0.");
                }
            }

            if (area > 0)
            {
                MessageBox.Show($"El área calculada es: {area}", "Resultado", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }

        // Método para validar las entradas numéricas
